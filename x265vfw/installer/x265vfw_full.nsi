;x265vfw - H.265/MPEG-H codec
;Written by BugMaster & Pidi

!define FullName    "x265vfw - H.265/MPEG-H codec"
!define ShortName   "x265vfw"
!define ShortName64 "x265vfw64"
...
Section "Remove previous version" SEC_REMOVE_PREV
  ...
  ${If} $PREV_INSTDIR != ""
    ReadRegDWORD $R0 HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${ShortName}" "Installed_x265vfw_x86"
    StrCmp $R0 "" +1 +2
    StrCpy $R0 1
    ...
    ReadRegDWORD $R0 HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${ShortName}" "Installed_x265vfw_x64"
    StrCmp $R0 "" +1 +2
    StrCpy $R0 0
    ...
    ${If} $R0 <> 0
      ${If} ${RunningX64}
        ...
        Delete "$PREV_INSTDIR\x265vfw64.dll"
        Delete "$PREV_INSTDIR\x265vfw64.ico"
  ...
  ${If} $PREV_INSTDIR64 != ""
    ${If} ${RunningX64}
      ...
      Delete "$PREV_INSTDIR64\x265vfw64.dll"
      Delete "$PREV_INSTDIR64\x265vfw64.ico"
...
Section "-Install"
  ...
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${ShortName}" "Installed_x265vfw_x86" 0
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${ShortName}" "Installed_x265vfw_x64" 0
...
Section "${ShortName} (x86)" SEC_INSTALL_X86
  ...
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${ShortName}" "Installed_x265vfw_x86" 1
  ...
  ${If} ${FileExists} "$SYSDIR_32bit\rundll32.exe"
    CreateShortcut "$SMPROGRAMS\${ShortName}\Configure ${ShortName}.lnk" "$SYSDIR_32bit\rundll32.exe" "x265vfw.dll,Configure" "$INSTDIR\x265vfw.ico"
  ${Else}
    CreateShortcut "$SMPROGRAMS\${ShortName}\Configure ${ShortName}.lnk" "rundll32.exe" "x265vfw.dll,Configure" "$INSTDIR\x265vfw.ico"
  ${EndIf}

  !insertmacro RegCodec "vidc.x265" "x265vfw.dll" "${FullName}"

SectionEnd

Section "${ShortName} (x64)" SEC_INSTALL_X64

  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${ShortName}" "Installed_x265vfw_x64" 1

  SetOutPath $INSTDIR
  File /oname=x265vfw64.ico ".\x265vfw.ico"
  ...
  ${If} ${FileExists} "$SYSDIR\rundll32.exe"
    CreateShortcut "$SMPROGRAMS\${ShortName}\Configure ${ShortName64}.lnk" "$SYSDIR\rundll32.exe" "x265vfw64.dll,Configure" "$INSTDIR\x265vfw64.ico"
  ${Else}
    CreateShortcut "$SMPROGRAMS\${ShortName}\Configure ${ShortName64}.lnk" "rundll32.exe" "x265vfw64.dll,Configure" "$INSTDIR\x265vfw64.ico"
  ${EndIf}

  ${EnableX64FSRedirection}

  SetRegView 64
  !insertmacro RegCodec "vidc.x265" "x265vfw64.dll" "${FullName}"
  SetRegView lastused
...
Section "Uninstall"
  ...
  ReadRegDWORD $R8 HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${ShortName}" "Installed_x265vfw_x86"
  StrCmp $R8 "" +1 +2
  StrCpy $R8 1

  ReadRegDWORD $R9 HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${ShortName}" "Installed_x265vfw_x64"
  StrCmp $R9 "" +1 +2
  StrCpy $R9 0
  ...
  ${If} $R0 <> 0
    ...
    ${If} $R8 <> 0
      DeleteRegKey HKCU "Software\GNU\x265"
    ${EndIf}
    ${If} $R9 <> 0
      ${If} ${RunningX64}
        SetRegView 64
        DeleteRegKey HKCU "Software\GNU\x265vfw64"
        SetRegView lastused
      ${EndIf}
    ${EndIf}
keep_settings:
  ...
  ${If} $R9 <> 0
    ${If} ${RunningX64}
      ...
      Delete /REBOOTOK "$SYSDIR\x265vfw64.dll"
      ...
      Delete /REBOOTOK "$INSTDIR\x265vfw64.dll"
      Delete /REBOOTOK "$INSTDIR\x265vfw64.ico"

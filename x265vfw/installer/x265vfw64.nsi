;x265vfw - H.265/MPEG-H codec for x64
;Written by BugMaster & Pidi

!define FullName  "x265vfw - H.265/MPEG-H codec for x64"
!define ShortName "x265vfw64"
...
Section "-Install"
  ...
  ${If} ${FileExists} "$SYSDIR\rundll32.exe"
    CreateShortcut "$SMPROGRAMS\${ShortName}\Configure ${ShortName}.lnk" "$SYSDIR\rundll32.exe" "x265vfw64.dll,Configure" "$INSTDIR\x265vfw64.ico"
  ${Else}
    CreateShortcut "$SMPROGRAMS\${ShortName}\Configure ${ShortName}.lnk" "rundll32.exe" "x265vfw64.dll,Configure" "$INSTDIR\x265vfw64.ico"
  ${EndIf}
...
keep_settings:
  ...
  Delete /REBOOTOK "$SYSDIR\libx265.dll"

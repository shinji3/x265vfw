...
static int convert_fail( x265_picture *img_dst, x265_picture *img_src,
...
static int name( x265_picture *img_dst, x265_picture *img_src,                   \
                 int i_width, int i_height )                                     \
{                                                                                \
    if( img_src->colorSpace & X264VFW_CSP_VFLIP )                                     \
    {                                                                            \
        plane_copy_vflip( img_dst->planes[0], img_dst->stride[0],               \
                          img_src->planes[0], img_src->stride[0],               \
                          i_width, i_height );                                   \
        plane_##func##_vflip( img_dst->planes[1+swap], img_dst->stride[1+swap], \
                              img_src->planes[1], img_src->stride[1],           \
                              i_width >> h_shift, i_height >> v_shift );         \
        plane_##func##_vflip( img_dst->planes[2-swap], img_dst->stride[2-swap], \
                              img_src->planes[2], img_src->stride[2],           \
                              i_width >> h_shift, i_height >> v_shift );         \
    }                                                                            \
    else                                                                         \
    {                                                                            \
        plane_copy( img_dst->planes[0], img_dst->stride[0],                     \
                    img_src->planes[0], img_src->stride[0],                     \
                    i_width, i_height );                                         \
        plane_##func( img_dst->planes[1+swap], img_dst->stride[1+swap],         \
                      img_src->planes[1], img_src->stride[1],                   \
                      i_width >> h_shift, i_height >> v_shift );                 \
        plane_##func( img_dst->planes[2-swap], img_dst->stride[2-swap],         \
                      img_src->planes[2], img_src->stride[2],                   \
                      i_width >> h_shift, i_height >> v_shift );                 \
    }                                                                            \
    return 0;                                                                    \
...
{                                                                      \
    ...
    {                                                                  \
        plane_copy_vflip( img_dst->planes[0], img_dst->stride[0],     \
                          img_src->planes[0], img_src->stride[0],     \
                          i_width, i_height );                         \
        plane_##func##_vflip( img_dst->planes[1], img_dst->stride[1], \
                              img_src->planes[1], img_src->stride[1], \
    ...
    {                                                                  \
        plane_copy( img_dst->planes[0], img_dst->stride[0],           \
                    img_src->planes[0], img_src->stride[0],           \
                    i_width, i_height );                               \
        plane_##func( img_dst->planes[1], img_dst->stride[1],         \
                      img_src->planes[1], img_src->stride[1],         \
                      i_width, i_height >> v_shift );                  \
    }                                                                  \
    return 0;                                                          \
...
static int name( x265_picture *img_dst, x265_picture *img_src, \
                 int i_width, int i_height )                   \
{                                                              \
    uint8_t *src = img_src->planes[0];                          \
    int     i_src= img_src->stride[0];                       \
                                                               \
    uint8_t *y   = img_dst->planes[0];                          \
    uint8_t *u   = img_dst->planes[1];                          \
    uint8_t *v   = img_dst->planes[2];                          \
                                                               \
    if( img_src->colorSpace & X264VFW_CSP_VFLIP )                   \
    {                                                          \
...
static int name##_##rec##_##scale( x265_picture *img_dst, x265_picture *img_src,  \
                                   int i_width, int i_height )                    \
{                                                                                 \
    uint8_t *src = img_src->planes[0];                                             \
    int     i_src= img_src->stride[0];                                          \
    int     i_y  = img_dst->stride[0];                                          \
    uint8_t *y   = img_dst->planes[0];                                             \
    uint8_t *u   = img_dst->planes[1];                                             \
    uint8_t *v   = img_dst->planes[2];                                             \
                                                                                  \

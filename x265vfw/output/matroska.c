...
static int set_param( hnd_t handle, x265_param *p_param )
{
    ...
    if( p_param->fpsNum > 0 /* && !p_param->b_vfr_input */)
    {
        p_mkv->frame_duration = (int64_t)p_param->fpsDenom *
                                (int64_t)1000000000 / p_param->fpsNum;
    }
    else
    {
    ...
    {
        if ( p_param->vui.sarWidth > p_param->vui.sarHeight ) {
            dw = dw * p_param->vui.sarWidth / p_param->vui.sarHeight;
        } else {
            dh = dh * p_param->vui.sarHeight / p_param->vui.sarWidth;
        }
    }
    p_mkv->d_width = (int)dw;
    p_mkv->d_height = (int)dh;

    p_mkv->i_timebase_num = p_param->fpsDenom;
    p_mkv->i_timebase_den = p_param->fpsNum;
...
static int write_headers( hnd_t handle, x265_nal *p_nal )
{
    ...
    int sei_size = p_nal[3].sizeBytes;
    ...
    ret = mk_write_header( p_mkv->w, x265_version_str, "V_MPEGH/ISO/HEVC",
...
static int write_frame( hnd_t handle, uint8_t *p_nalu, int i_size, x265_picture *p_picture )
{
    ...
    int64_t i_stamp = (int64_t)((p_picture->pts * 1e9 * p_mkv->i_timebase_num / p_mkv->i_timebase_den) + 0.5);

    p_mkv->b_writing_frame = 0;

    if( mk_set_frame_flags( p_mkv->w, i_stamp, IS_X265_TYPE_I(p_picture->frameData.sliceType), IS_X265_TYPE_B(p_picture->frameData.sliceType)) < 0 )
        return -1;

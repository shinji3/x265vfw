...
static int open_file( char *psz_filename, hnd_t *p_handle, cli_output_opt_t *opt )
{
    *p_handle = NULL;
    ...
    memcpy( &p_flv->opt, opt, sizeof(cli_output_opt_t) );
...
static int set_param( hnd_t handle, x265_param *p_param )
{
    ...
    flv_put_amf_double( c, p_param->sourceWidth );

    flv_put_amf_string( c, "height" );
    flv_put_amf_double( c, p_param->sourceHeight );

    flv_put_amf_string( c, "framerate" );

 /*   if( !p_param->b_vfr_input ) */
        flv_put_amf_double( c, (double)p_param->fpsNum / p_param->fpsDenom );
/*    else
    {
    ...
    flv_put_amf_double( c, FLV_CODECID_HEVC );
    ...
    p_flv->i_fps_num = p_param->fpsNum;
    p_flv->i_fps_den = p_param->fpsDenom;
    p_flv->d_timebase = (double)1.0; /* p_param->i_timebase_num / p_param->i_timebase_den; */
    p_flv->b_vfr_input = 0; /* p_param->b_vfr_input; */
    p_flv->i_delay_frames = p_param->bframes ? (p_param->bBPyramid ? 2 : 1) : 0;
...
static int write_headers( hnd_t handle, x265_nal *p_nal )
{
    ...
    int sps_size = p_nal[SPS_POS].sizeBytes;
    int pps_size = p_nal[PPS_POS].sizeBytes;
    int sei_size = p_nal[SEI_POS].sizeBytes;
    ...
    memcpy( p_flv->sei, p_nal[SEI_POS].payload, sei_size );

    // SPS
    uint8_t *sps = p_nal[SPS_POS].payload + 4;
    ...
    flv_put_byte( c, FLV_CODECID_HEVC | FLV_FRAME_KEY ); // Frametype and CodecID
    ...
    flv_append_data( c, p_nal[PPS_POS].payload + 4, pps_size - 4 );
...
#define convert_timebase_ms( timestamp, timebase ) (int64_t)((timestamp) * (timebase) * 1000 + 0.5)
    ...
    if( !p_flv->i_framenum )
    {
        p_flv->i_delay_time = p_picture->dts * -1;
        if( !p_flv->opt.use_dts_compress && p_flv->i_delay_time )
            x264vfw_cli_log( p_flv->opt.p_private, "flv", X265_LOG_WARNING, "initial delay %llu ms\n",
                             convert_timebase_ms( p_picture->pts + p_flv->i_delay_time, p_flv->d_timebase ) );
    }
    ...
    if( p_flv->opt.use_dts_compress )
    {
        if( p_flv->i_framenum == 1 )
            p_flv->i_init_delta = convert_timebase_ms( p_picture->dts + p_flv->i_delay_time, p_flv->d_timebase );
        dts = p_flv->i_framenum > p_flv->i_delay_frames
            ? convert_timebase_ms( p_picture->dts, p_flv->d_timebase )
            : p_flv->i_framenum * p_flv->i_init_delta / (p_flv->i_delay_frames + 1);
        cts = convert_timebase_ms( p_picture->pts, p_flv->d_timebase );
    }
    else
    {
        dts = convert_timebase_ms( p_picture->dts + p_flv->i_delay_time, p_flv->d_timebase );
        cts = convert_timebase_ms( p_picture->pts + p_flv->i_delay_time, p_flv->d_timebase );
    }
    offset = cts - dts;
    ...
    if( p_flv->i_framenum )
    {
        if( p_flv->i_prev_dts == dts )
            x264vfw_cli_log( p_flv->opt.p_private, "flv", X265_LOG_WARNING, "duplicate DTS %llu generated by rounding\n"
                             "               decoding framerate cannot exceed 1000fps\n", dts );
        if( p_flv->i_prev_cts == cts )
            x264vfw_cli_log( p_flv->opt.p_private, "flv", X265_LOG_WARNING, "duplicate CTS %llu generated by rounding\n"
    ...
    p_flv->start = c->d_cur;
    flv_put_byte( c, IS_X265_TYPE_I(p_picture->frameData.sliceType)? FLV_FRAME_KEY : FLV_FRAME_INTER );

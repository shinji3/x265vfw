...
static int open_file( char *psz_filename, hnd_t *p_handle, cli_output_opt_t *opt )
{
    *p_handle = NULL;
    ...
    if( b_regular )
    {
        ...
        if( !fh )
        {
            x264vfw_cli_log( opt->p_private, "avi", X265_LOG_ERROR, "cannot open output file `%s'.\n", psz_filename );
            return -1;
        }
    ...
    if( !b_regular )
    {
        x264vfw_cli_log( opt->p_private, "avi", X265_LOG_ERROR, "AVI output is incompatible with non-regular file `%s'\n", psz_filename );
        return -1;
    }
...
static int set_param( hnd_t handle, x265_param *p_param )
{
    ...
    c = h->video_stm->codec;
    c->flags |= p_param->bRepeatHeaders ? 0 : CODEC_FLAG_GLOBAL_HEADER;
    c->time_base.num = p_param->fpsDenom;
    c->time_base.den = p_param->fpsNum;
    c->width = p_param->sourceWidth;
    c->height = p_param->sourceHeight;
    c->pix_fmt = AV_PIX_FMT_NONE; //not used
    c->codec_type = AVMEDIA_TYPE_VIDEO;
    c->codec_id = AV_CODEC_ID_HEVC;
    c->codec_tag = MKTAG('H','E','V','C');

    h->video_stm->time_base.num = c->time_base.num;
    h->video_stm->time_base.den = c->time_base.den;
...
static int write_headers( hnd_t handle, x265_nal *p_nal )
{
    avi_hnd_t *h = handle;
    AVCodecContext *c;
    int i_size = p_nal[1].sizeBytes + p_nal[2].sizeBytes;
    ...
    {
        c->extradata_size = i_size;
        av_freep( &c->extradata );
        c->extradata = av_malloc( c->extradata_size );
        if( !c->extradata )
            return -1;
        /* Write the SPS/PPS to the extradata */
        memcpy( c->extradata, p_nal[1].payload, p_nal[1].sizeBytes );
        memcpy( c->extradata + p_nal[1].sizeBytes, p_nal[2].payload, p_nal[2].sizeBytes );
        /* Write the SEI as part of the first frame */
        if( write_buffer( h, p_nal[3].payload, p_nal[3].sizeBytes ) < 0 )
            return -1;
    ...
    {
        if( write_buffer( h, p_nal[1].payload, p_nal[1].sizeBytes ) < 0 )
            return -1;
        if( write_buffer( h, p_nal[2].payload, p_nal[2].sizeBytes ) < 0 )
            return -1;
        if( write_buffer( h, p_nal[3].payload, p_nal[3].sizeBytes ) < 0 )
            return -1;
    }
...
static int write_frame( hnd_t handle, uint8_t *p_nalu, int i_size, x265_picture *p_picture )
{
    ...
    pkt.flags |= IS_X265_TYPE_I(p_picture->frameData.sliceType)? AV_PKT_FLAG_KEY : 0;
    if( h->d_cur )
    {
    ...
    pkt.pts = av_rescale_q( p_picture->pts, h->video_stm->codec->time_base, h->video_stm->time_base );
    pkt.dts = av_rescale_q( p_picture->dts, h->video_stm->codec->time_base, h->video_stm->time_base );

...
#define MP4_LOG_ERROR( ... )                x264vfw_cli_log( p_mp4->opt.p_private, "mp4", X265_LOG_ERROR, __VA_ARGS__ )
#define MP4_LOG_WARNING( ... )              x264vfw_cli_log( p_mp4->opt.p_private, "mp4", X265_LOG_WARNING, __VA_ARGS__ )
#define MP4_LOG_INFO( ... )                 x264vfw_cli_log( p_mp4->opt.p_private, "mp4", X265_LOG_INFO, __VA_ARGS__ )
...
if( cond )\
{\
    x264vfw_cli_log( opt->p_private, "mp4", X265_LOG_ERROR, __VA_ARGS__ );\
    return -1;\
}
...
static int open_file( char *psz_filename, hnd_t *p_handle, cli_output_opt_t *opt )
{
    *p_handle = NULL;
    ...
    MP4_FAIL_IF_ERR_EX2( lsmash_open_file( psz_filename, 0, &p_mp4->file_param ) < 0, "failed to open an output file.\n" );
    if( p_mp4->b_fragments )
...
static int set_param( hnd_t handle, x265_param *p_param )
{
    ...
    p_mp4->i_delay_frames = p_param->bframes ? (p_param->bBPyramid ? 2 : 1) : 0;
    ...
    MP4_FAIL_IF_ERR( i_media_timescale > UINT32_MAX, "MP4 media timescale %llu exceeds maximum\n", i_media_timescale );
    ...
    p_mp4->summary->width = p_param->sourceWidth;
    p_mp4->summary->height = p_param->sourceHeight;
    ...
    if( p_param->vui.sarWidth && p_param->vui.sarHeight )
    {
        ...
        p_mp4->summary->par_h = p_param->vui.sarWidth;
        p_mp4->summary->par_v = p_param->vui.sarHeight;
    }
    p_mp4->summary->color.primaries_index = p_param->vui.colorPrimaries;
    p_mp4->summary->color.transfer_index  = p_param->vui.transferCharacteristics;
    p_mp4->summary->color.matrix_index    = p_param->vui.matrixCoeffs >= 0 ? p_param->vui.matrixCoeffs : ISOM_MATRIX_INDEX_UNSPECIFIED;
    p_mp4->summary->color.full_range      = p_param->vui.bEnableVideoFullRangeFlag >= 0 ? p_param->vui.bEnableVideoFullRangeFlag : 0;
    ...
    if( p_mp4->b_use_recovery )
    {
        media_param.roll_grouping = p_param->bIntraRefresh;
        media_param.rap_grouping = p_param->bOpenGOP;
    }
...
static int write_headers( hnd_t handle, x265_nal *p_nal )
{
    ...
    uint32_t sps_size = p_nal[0].sizeBytes - H264_NALU_LENGTH_SIZE;
    uint32_t pps_size = p_nal[1].sizeBytes - H264_NALU_LENGTH_SIZE;
    uint32_t sei_size = p_nal[2].sizeBytes;

    uint8_t *sps = p_nal[0].payload + H264_NALU_LENGTH_SIZE;
    uint8_t *pps = p_nal[1].payload + H264_NALU_LENGTH_SIZE;
    uint8_t *sei = p_nal[2].payload;
...
static int write_frame( hnd_t handle, uint8_t *p_nalu, int i_size, x265_picture *p_picture )
{
    ...
    if( !p_mp4->i_numframe )
    {
        p_mp4->i_start_offset = p_picture->dts * -1;
    ...
    if( p_mp4->opt.use_dts_compress )
    {
        if( p_mp4->i_numframe == 1 )
            p_mp4->i_init_delta = (p_picture->dts + p_mp4->i_start_offset) * p_mp4->i_time_inc;
        dts = p_mp4->i_numframe > p_mp4->i_delay_frames
            ? p_picture->dts * p_mp4->i_time_inc
            : p_mp4->i_numframe * (p_mp4->i_init_delta / p_mp4->i_dts_compress_multiplier);
        cts = p_picture->pts * p_mp4->i_time_inc;
    }
    else
    {
        dts = (p_picture->dts + p_mp4->i_start_offset) * p_mp4->i_time_inc;
        cts = (p_picture->pts + p_mp4->i_start_offset) * p_mp4->i_time_inc;
    }

    p_sample->dts = dts;
    p_sample->cts = cts;
    p_sample->index = p_mp4->i_sample_entry;
    p_sample->prop.ra_flags = IS_X265_TYPE_I(p_picture->frameData.sliceType)? ISOM_SAMPLE_RANDOM_ACCESS_FLAG_SYNC : ISOM_SAMPLE_RANDOM_ACCESS_FLAG_NONE;

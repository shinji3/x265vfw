...
#ifdef _WIN64
#define X264VFW_REG_CHILD  "x265vfw64"
#else
/* Not "x264vfw" because of GordianKnot compatibility */
#define X264VFW_REG_CHILD  "x265"
#endif
#define X264VFW_REG_CLASS  "config"

/* Description */
#define X264VFW_NAME       "x265vfw"
...
static const reg_int_t reg_int_table[] =
{
    ...
    { "slow1pass",       &reg.b_slow1pass,       0,   0,  1                    },
...
static void dlg_update_items(CONFIG_DATA *cfg_data)
{
    ...
    if (SendMessage(GetDlgItem(hMainDlg, IDC_RC_MODE), CB_GETCOUNT, 0, 0) == 0)
    {
        ...
        SendDlgItemMessage(hMainDlg, IDC_RC_MODE, CB_ADDSTRING, 0, (LPARAM)"Multipass - 1st pass (slow)");
    ...
    /* Encoder */
    sprintf(temp, "libx265 core %d%d", X265_BUILD, X265_MAJOR_VERSION);
                ...
                case BN_CLICKED:
                    switch (LOWORD(wParam))
                    {
                        ...
                        case IDC_OUTPUT_BROWSE:
                        {
                            ...
                            ofn.lpstrFilter = L"*.avi\0*.avi\0*.flv\0*.flv\0*.mkv\0*.mkv\0*.mp4\0*.mp4\0*.h265\0*.h265\0All files (*.*)\0*.*\0\0";
                            ofn.nFilterIndex = tmp[0] ? get_extension_index(tmp, L"avi\0flv\0mkv\0mp4\0h265\0\0") : 1;
#else
                            ofn.lpstrFilter = L"*.flv\0*.flv\0*.mkv\0*.mkv\0*.mp4\0*.mp4\0*.h265\0*.h265\0All files (*.*)\0*.*\0\0";
                            ofn.nFilterIndex = tmp[0] ? get_extension_index(tmp, L"flv\0mkv\0mp4\0h265\0\0") : 1;
#endif
...
INT_PTR CALLBACK x264vfw_callback_about(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            ...
            sprintf(temp, "Build date: %s %s\nlibx265 core %d%d", __DATE__, __TIME__, X265_BUILD, X265_MAJOR_VERSION);
...
#ifndef X265_BIT_DEPTH
#define X265_BIT_DEPTH 8
#endif
...
#if 0
    char buf[50];
    x265_param defaults_value;
    x265_param *defaults = &defaults_value;

    x264_param_default(&defaults_value);
    /* Set VFW defaults for colorspace options */
    defaults->vui.bEnableVideoFullRangeFlag = 0; /* tv range */
    #if 0
